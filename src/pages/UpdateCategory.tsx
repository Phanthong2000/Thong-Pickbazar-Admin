...
import { allCategoriesSelector, getApiAllCategories } from '../redux/slices/categorySlice';
...
function UpdateCategory() {
    ...
    const getCategory = async (id: string) => {
        try {
            ...
            if (result) {
                ...
                setValue("group", result.groupId);
                setValue("parent", result.parentId);
    ...
    const onSubmit = (data: any) => {
        if (image.length === 0 && newImage.length === 0) setErrorImage("Must choose image");
        else {
    ...
    const saveCategory = async (category: any) => {
        let imageUrl = image.at(0);
        if (newImage.length > 0) imageUrl = await saveImage("categories", newImage[0]);
        const body = {
            ...category,
            image: imageUrl
        }
        ...
        if (result) {
            ...
            dispatch(themeSlice.actions.showToast({
                content: "Successfully Update category",
                type: "success"
            }));
            navigate("/categories");
        }
    ...
    return (
        <>
            <form onSubmit={handleSubmit(onSubmit)}>
                <div className="font20 font_family_bold mt-2">Update Category</div>
                ...
                <div className="row p-0 m-0 mt-4">
                    ...
                    <div className="col-12 col-lg-8 box_shadow_card bg_white border_radius_5 p-4 w100_per">
                        ...
                        <div className="d-flex flex-wrap mt-2">
                            ...
                            {
                                newImage.map((item: any, index: number) => (
                                    <BaseFileChosen
                                        file={true}
                ...
                <div className="row p-0 m-0 mt-4">
                    ...
                    <div className="col-12 col-lg-8 box_shadow_card bg_white border_radius_5 p-4 w100_per">
                        ...
                        <div className="font_family_bold_italic font14 mt-4">Name Vietnamese</div>
                        <input
                            {...register("name_vi")}
                            className="mt-2 h40_px w100_per"
                        ...
                        <Select
                            value={
                                options.filter((option) => option.value === group)
                            }
                            styles={{
                            ...
                            isDisabled={!parent}
                            placeholder="Choose Group" onChange={(value) => handleChooseGroup(value)} options={options} />
                        <div className="mt-2 font12 ml_5px color_red font_family_italic">
                            {errors.group?.message}
                        </div>
                        <div className="font_family_bold_italic font14 mt-4">Parent Category</div>
                        <Select
                            value={
                                optionsParent.filter((option) => option.value === parent)
                            }
                            styles={{
                                control: (provided, state) => ({
                                    ...provided,
                            ...
                            isDisabled={!parent}
                            placeholder="Choose parent" onChange={(value) => handleChooseParent(value)} options={optionsParent} />
